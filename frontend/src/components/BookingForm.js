...
const facilityColors = { Gym: "#e67e22", Basketball: "#2980b9", Badminton: "#27ae60", "Table Tennis": "#8e44ad" };

function generateTimeSlots() {
  const slots = [];
  for (let hour = 10; hour < 13; hour++) { // 10 AM to 1 PM
  ...
  for (let hour = 14; hour < 16; hour++) { // 2 PM to 4 PM
    slots.push({ start: `${String(hour).padStart(2, '0')}:00`, end: `${String(hour).padStart(2, '0')}:30` });
    slots.push({ start: `${String(hour).padStart(2, '0')}:30`, end: `${String(hour + 1).padStart(2, '0')}:00` });
  }
  return slots;
...
function BookingForm() {
  ...
  const handleBooking = async () => {
    ...
    try {
        ...
        // --- FIXED: Use the response from the POST request directly ---
        // This ensures the QR code is displayed immediately.
  ...
  const handleCancel = async () => {
    ...
    try {
        ...
        setCurrentBooking(null); // Clear the booking from the UI
    } catch (error) {
        handleApiError(error);
    }
  };
    ...
    return (
      <div style={styles.container}>
        <div style={styles.cooldownBox}>
          ...
          <p>You can book again after: <strong>{new Date(cooldownUntil).toLocaleString()}</strong></p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={styles.container}>
      ...
      {message && <p style={{...styles.message, color: message.toLowerCase().includes("error") || message.toLowerCase().includes("cooldown") ? "#e74c3c" : "#27ae60"}}>{message}</p>}
      
      {currentBooking ? (
        <div style={styles.bookingInfo}>
          <h3>Your Active Booking</h3>
          <p><strong>{currentBooking.facility}</strong> at <strong>{new Date(currentBooking.start).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</strong></p>
          {currentBooking.qr_code_base64 && <img src={`data:image/png;base64,${currentBooking.qr_code_base64}`} alt="Booking QR Code" style={{maxWidth: '200px', margin: '1rem auto', display: 'block'}}/>}
        ...
        <>
          <div style={styles.facilityTabs}>
            {facilities.map(f => (
              <button key={f} onClick={() => setSelectedFacility(f)} style={{...styles.facilityTab, backgroundColor: selectedFacility === f ? facilityColors[f] : "transparent", color: selectedFacility === f ? "white" : facilityColors[f], borderColor: facilityColors[f]}}>{f}</button>
            ))}
          </div>
          <div style={styles.slotsGrid}>
            {timeSlots.map((slot, idx) => {
              const isBooked = isSlotBooked(selectedFacility, slot);
              return <button key={idx} disabled={isBooked} onClick={() => setSelectedSlotIdx(idx)} style={{...styles.slotButton, backgroundColor: isBooked ? "#bdc3c7" : selectedSlotIdx === idx ? facilityColors[selectedFacility] : "#2ecc71", cursor: isBooked ? "not-allowed" : "pointer"}}>{slot.start}</button>;
            })}
          </div>
          <button onClick={handleBooking} disabled={selectedSlotIdx === null} style={{...styles.bookBtn, backgroundColor: selectedSlotIdx !== null ? facilityColors[selectedFacility] : "#bdc3c7"}}>Book Selected Slot</button>
        </>
      )}
    </div>
  );
...
const styles = {
  ...
  slotsGrid: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(100px, 1fr))", gap: "10px", marginBottom: "1.5rem" },
  slotButton: { width: "100%", height: "45px", borderRadius: "8px", border: "none", color: "white", fontWeight: "600", cursor: "pointer" },
